...
Deno.test("Text 1 512", { permissions: "none" }, () => {
...
Deno.test("Text 1 1024", { permissions: "none" }, () => {
...
Deno.test("Text 6 512", { permissions: "none" }, () => {
...
Deno.test("Text 6 1024", { permissions: "none" }, () => {
...
Deno.test("Text 12 32", { permissions: "none" }, () => {
	const instance = new FNV1a(32, sample12);
	deepStrictEqual(instance.hash(), 3_582_672_807n);
	deepStrictEqual(instance.hashHex(), "D58B3FA7");
});
Deno.test("Text 12 64", { permissions: "none" }, () => {
	deepStrictEqual(new FNV1a(64, sample12).hashHex(), "779A65E7023CD2E7");
});
Deno.test("Text 12 128", { permissions: "none" }, () => {
	deepStrictEqual(new FNV1a(128, sample12).hashHex(), "6C155799FDC8EEC4B91523808E7726B7");
});
Deno.test("Text 12 256", { permissions: "none" }, () => {
	deepStrictEqual(new FNV1a(256, sample12).hashHex(), "ECC3CF2E0EDFCCD3D87F21EC0883AAD4DB43EEAD66CE09EB4A97E04E1A184527");
});
Deno.test("Text 12 512", { permissions: "none" }, () => {
	deepStrictEqual(new FNV1a(512, sample12).hashHex(), "2B9C19EC56CCF98DA0F227CC82BFAACBD8350928BD2CEACAE7BC8AA13E747F5C43CA4E2E98FC25E94E4E805675545EE95A3B968C0ACFAECB90AEA2FDBCD4DE0F");
});
Deno.test("Text 12 1024", { permissions: "none" }, () => {
	deepStrictEqual(new FNV1a(1024, sample12).hashHex(), "");
});
...
Deno.test("Text 14", { permissions: "none" }, () => {
	...
	deepStrictEqual(instance.hashHex(), "AAF5FEE7");
});
Deno.test("Text 15", { permissions: "none" }, () => {
...
Deno.test("Stream 1 32", {
	...
	const hashFromText = new FNV1a(32, sampleText).hash();
	await using sampleFile = await Deno.open(sampleFilePath);
	const hashFromStream = (await new FNV1a(32).updateFromStream(sampleFile.readable)).hash();
...
Deno.test("Stream 1 64", {
	...
	const sampleFilePath = "./README.md";
	const sampleText = await Deno.readTextFile(sampleFilePath);
	const hashFromText = new FNV1a(64, sampleText).hash();
	await using sampleFile = await Deno.open(sampleFilePath);
	const hashFromStream = (await new FNV1a(64).updateFromStream(sampleFile.readable)).hash();
	deepStrictEqual(hashFromText, hashFromStream);
});
Deno.test("Stream 1 128", {
	...
	const sampleFilePath = "./README.md";
	const sampleText = await Deno.readTextFile(sampleFilePath);
	const hashFromText = new FNV1a(128, sampleText).hash();
	await using sampleFile = await Deno.open(sampleFilePath);
	const hashFromStream = (await new FNV1a(128).updateFromStream(sampleFile.readable)).hash();
	deepStrictEqual(hashFromText, hashFromStream);
});
Deno.test("Stream 1 256", {
	...
	const sampleFilePath = "./README.md";
	const sampleText = await Deno.readTextFile(sampleFilePath);
	const hashFromText = new FNV1a(256, sampleText).hash();
	await using sampleFile = await Deno.open(sampleFilePath);
	const hashFromStream = (await new FNV1a(256).updateFromStream(sampleFile.readable)).hash();
	deepStrictEqual(hashFromText, hashFromStream);
});
Deno.test("Stream 1 512", {
	...
	const sampleFilePath = "./README.md";
	const sampleText = await Deno.readTextFile(sampleFilePath);
	const hashFromText = new FNV1a(512, sampleText).hash();
	await using sampleFile = await Deno.open(sampleFilePath);
	const hashFromStream = (await new FNV1a(512).updateFromStream(sampleFile.readable)).hash();
	deepStrictEqual(hashFromText, hashFromStream);
});
Deno.test("Stream 1 1024", {
	...
	const sampleFilePath = "./README.md";
	const sampleText = await Deno.readTextFile(sampleFilePath);
	const hashFromText = new FNV1a(1024, sampleText).hash();
	await using sampleFile = await Deno.open(sampleFilePath);
	const hashFromStream = (await new FNV1a(1024).updateFromStream(sampleFile.readable)).hash();
	deepStrictEqual(hashFromText, hashFromStream);
});
